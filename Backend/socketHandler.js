...
const setupSocket = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    // Personal channel for automated reminder pushes (see jobs/reminderScheduler.js)
    socket.on("join-user-reminders", ({ userId }) => {
      if (!userId) return;
      const room = `user:${userId}`;
      socket.join(room);
      socket.reminderUserRoom = room;
    });
    ...
    socket.on("disconnect", () => {
      ...
      if (socket.reminderUserRoom) {
        socket.leave(socket.reminderUserRoom);
      }
